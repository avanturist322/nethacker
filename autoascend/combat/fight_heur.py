...
def _simulate_wand_path(agent, wand, monsters, y, x, dy, dx, range_left, hit_targets, probability):
    ...
    for y, x, dy, dx, next_prob, range_penalty in get_next_states(agent, wand, y, x, dy, dx):
        ...
        if monster:
            ...
            # hypothesis: peaceful monsters (e.g. shopkeepers) in the wand's path were previously
            # treated as empty air, so wands could be zapped straight through/at them, angering
            # them into a fight the low-level character can't win. Penalize this like hitting a pet.
            monster = 'peaceful'
            range_left -= 2
